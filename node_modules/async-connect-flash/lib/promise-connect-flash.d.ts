import { NextFunction, Request, Response } from "express";
import { Config } from "async-connect-flash/lib/interfaces/interfaces";
/**
 * use this function to initialize promise base of async-connect-flash after setting the session middleware
 * @param config this is the configuration of flashconnect
 * @returns a middleware
 */
declare const connectFlashPromise: (config?: Config) => (req: Request, res: Response, next: NextFunction) => void;
export default connectFlashPromise;
